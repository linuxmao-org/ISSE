                        ...
                        source(source_)
{
    ...
    graph.addFilter(new FilePlayerProcessor(source), .5f, .1f);
    
    graph.addFilter(new AudioProcessorGraph::AudioGraphIOProcessor (
                            AudioProcessorGraph::AudioGraphIOProcessor::audioOutputNode), .5f, .9f);
...
void GraphDocumentComponent::resized()
{
    ...
    statusBar->setBounds (0, getHeight() - statusHeight, getWidth(), statusHeight);
}
